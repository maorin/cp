...
def request_content(start, end):
    url_link = 'https://datachart.500.com/dlt/history/newinc/history.php?start={0}&end={1}'.format(start, end)
    headers = {
        'User-Agent': generate_user_agent(device_type='desktop', os=('mac', 'linux', 'win', 'android'))
    }
    print url_link
    response = get(url_link, timeout=6)
...
def fetch_all():
    file = io.open('dlt.txt', 'w', encoding='utf-8')
