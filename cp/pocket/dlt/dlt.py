...
from fetchdlt import fetch_all
...
class DLT():
    ...
    def compare_data(self, r, p):
        for i in range(len(r)):
            ...
            print "前区 %s星  后区 %s星" % (len(f_out),len(b_out))
            print "前区%s 后区%s  real: %s  p: %s" % (f_out, b_out, r[i], p[i])
    ...
    def start(self):
        ...
        result = []
        for i in range(7):
            result.append(round(self.run(i)))
        print result
    ...
    def run(self, n):
        ...
        X_train, y_train, last_data = self.load_data('dlt.txt' , seq_len, True, n)
        #print last_data
